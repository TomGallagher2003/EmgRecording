...
class ExerciseApp:
    ...
    def show_image(self, path):
        img = Image.open(path)
        max_w, max_h = WINDOW_WIDTH*0.7, WINDOW_HEIGHT//2.3
    ...
    def show_next_image(self, path):
        img = Image.open(path)
        max_w, max_h = WINDOW_WIDTH*0.7//2, WINDOW_HEIGHT//2.3//2
        img.thumbnail((max_w, max_h), Image.LANCZOS)
        tkimg = ImageTk.PhotoImage(img)
    ...
    def update_index(self):
        num = self.index_offset + self.current_index + 1
        rep = self.current_repeat + 1
        self.index_label.config(text=f"Movement: {num}, Repeat: {rep}")
    ...
    def run_cycle(self):
        ...
        if self.current_index < len(self.movement_images):
            if self.current_repeat == 0 and not self.after_last_repeat:
                duration = int(self.movement_delay * 1000)
                self.start_phase(duration, self.start_movement)
            ...
            duration = int(self.movement_delay * 1000)
            self.start_phase(duration, self.end_session)
            self.show_image(rest_image)
    ...
    def start_movement(self):
        if self.current_repeat < self.num_repeats:
            ...
            self.start_phase(duration, self.rest_after_movement)
        else:
            self.current_repeat = 0
            self.current_index += 1
            self.run_cycle()
    ...
    def stop_exercise(self):
        ...
        self._resume_countdown(int(self.movement_delay * 1000))

    def _resume_countdown(self, seconds_ms):
        if seconds_ms > 0:
            self.resume_button.config(text=f"Resume in {seconds_ms//1000}s", state='disabled')
            self.root.after(1000, self._resume_countdown, seconds_ms-1000)
        else:
            self.resume_button.config(text="Resume", state='normal')
    ...
    def end_session(self):
        ...
        self.runtime_label.config(text=f"Total Runtime: {total} s")
