...
class Session:
    ...
    def record(self, is_movement, rest_time, movement, perform_time=0, rep=None):
        ...
        if self.config.SAVE_COUNTERS:
            chans = []
            chans.append(data[self.config.SYNCSTATION_COUNTER_CHANNEL])
            if self.config.USE_EMG:
                chans.append(data[self.config.MUOVI_COUNTER_CHANNEL])
            if self.config.USE_EEG:
                chans.append(data[self.config.MUOVI_PLUS_COUNTER_CHANNEL])

            self.save_channels(np.array(chans), labels, "counters", perform_time, exercise_group,
                               suffix)
